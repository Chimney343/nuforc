...
class NUFORCGeocoder:
    def __init__(self, input, cache=None, address_columns=None, user_agent=f"{os.getenv('SYSTEM')}_nuforc_geocoder"):
        self.input = input
        self.cache = cache
    ...
    def make_address(self, street=None, city=None, state=None, state_abbreviation=None, country=None):
        address_parts = [part for part in [street, city, state, state_abbreviation, country] if
                         part not in [None, 'unparsed']]
        try:
    ...
    def run(self):
        ...
        self.input_df['raw_address'] = self.input_df.progress_apply(lambda x: self.make_address(x['city'], x['state'], x['state_abbreviation'], x['country']), axis=1)

        tqdm.pandas(desc="Geocoding.")
        df = pd.DataFrame(self.input_df['raw_address'].unique()).rename(columns={0: 'raw_address'})
        if self.cache:
            df = pd.merge(left=df, right=self.cache, on='raw_address')
            to_geocode_df = df[df['geocoded'].isna()]
            to_geocode_df['geocoded'] = to_geocode_df['raw_address'].progress_apply(self.geocode)
